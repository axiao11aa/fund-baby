...
import { ChevronIcon, CloseIcon, DragIcon, EditIcon, ExitIcon, EyeIcon, EyeOffIcon, GridIcon, ListIcon, MailIcon, MoonIcon, PinIcon, PinOffIcon, PlusIcon, RefreshIcon, SettingsIcon, SortIcon, StarIcon, SunIcon, SwitchIcon, TrashIcon, UpdateIcon, UserIcon } from "./components/Icons";
...
export default function HomePage() {
  ...
  const [processingTrades, setProcessingTrades] = useState(false);
  const processingTradesRef = useRef(false);
  const processPendingQueue = async () => {
    if (processingTradesRef.current) return;
    processingTradesRef.current = true;
    setProcessingTrades(true);
    try {
    ...
    try {
      ...
      if (rawFunds === null) {
        ...
        fetchFundData(defaultCode).then(data => {
          // A JSON import may complete while the first default quote is loading.
          if (localStorage.getItem('funds') !== null) return;
          setFunds([data]);
  ...
  const applyImportedData = data => {
    ...
    holdingsRef.current = data.holdings;
    setPendingTrades(data.pendingTrades);
    pendingTradesRef.current = data.pendingTrades;
    setRefreshMs(data.refreshMs);
    setTempSeconds(data.refreshMs / 1000);
    ...
    <div className="container content">
      <div className="navbar glass">
        ...
        <div className="actions">
          ...
          <button className="icon-button" aria-label="打开设置" title="设置" onClick={() => setSettingsOpen(true)}>
      ...
      <div className="footer">
        ...
        <div style={{ marginBottom: '12px' }}>
          数据保存在当前浏览器，可通过设置中的 JSON 导出、导入进行备份和迁移。
        </div>
      ...
      {settingsOpen && (
        <ModalBackdrop className="modal-overlay" role="dialog" aria-modal="true" aria-label="设置" onClick={() => setSettingsOpen(false)}>
          <div className="glass card modal" style={{ maxHeight: '88vh', overflowY: 'auto' }} onClick={(e) => e.stopPropagation()}>
            <div className="title" style={{ marginBottom: 12 }}>
              <SettingsIcon width="20" height="20" />
              <span>设置</span>
              <span className="muted">刷新频率与数据备份</span>
            </div>
            ...
            <BackupPanel onApplied={applyImportedData} busy={refreshing || loading || processingTrades} />
